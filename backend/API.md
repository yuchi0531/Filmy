...
本番環境では全エンドポイント（`/health` を除く）で API キー認証が必要です。
リクエストヘッダー `X-API-Key` に、サーバー側の `FILMY_API_KEY` と一致するキーを指定してください。

- 開発環境（`FILMY_ENVIRONMENT=development`、デフォルト）でキーが未設定（空文字）の場合、認証はスキップされます。
- 本番環境（`FILMY_ENVIRONMENT=production`）でキーが未設定（空文字）の場合、**fail-closed** として `503` を返し、
  設定漏れのまま全エンドポイントが公開されることを防ぎます。
- キーが不一致・欠落の場合は `401` が返ります。

### レート制限

IP ごとに 1 分あたりのリクエスト数を制限しています（デフォルト 60）。
上限を超えると `429` が返ります。制限値は環境変数 `FILMY_RATE_LIMIT_PER_MINUTE` で変更できます（0 以下で無効化）。
...
| --- | --- | --- |
| `200` | 成功 | — |
| `401` | 認証エラー（APIキー不一致・欠落） | `X-API-Key` ヘッダーなし/不正 |
| `404` | リソースが見つからない | 存在しない映画ID・ Filmarksの404相当ページ |
| `422` | バリデーションエラー（パラメータ不正） | 数字でない movie_id、空の q、radius 範囲外 |
| `429` | レート制限超過 | 1分あたりのリクエスト数が上限超過 |
| `503` | Filmarks が一時的に利用不能 / APIキー未設定（本番） | タイムアウト・403/429/5xx、アクセス不可ページ / `FILMY_ENVIRONMENT=production` で `FILMY_API_KEY` 未設定 |
